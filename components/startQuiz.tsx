...
const LOCAL_STORAGE_KEY = 'genealogyQuizState'
...
export default function StartQuiz() {
	...
	useEffect(() => {
		...
		const state = { currentQuestionIndex, filters, breadcrumb }
		localStorage.setItem(LOCAL_STORAGE_KEY, JSON.stringify(state))
	}, [currentQuestionIndex, filters, breadcrumb, hasMounted])
	...
	const toggleOption = (option: string) => {
		setFilters((prev) => {
			const selected = new Set(prev[currentQuestionIndex] || [])
			selected.has(option) ? selected.delete(option) : selected.add(option)
			return { ...prev, [currentQuestionIndex]: Array.from(selected) }
		})
	}
